...
RSpec.describe Lot, type: :model do
  describe "#valid?" do
    it "deve falhar se os campos forem passados incorretamente" do
      ...
      expect(error).to include "Data de termino não pode ficar em branco"
    ...
    it "data de inicio não pode ser no passado" do
      ...
      expect(error).to include "Data de início deve ser maior que #{Time.now}"
    end

    it "data de término não pode ser anterior a data de início" do
      ...
      expect(error).not_to eq []
      expect(error).to include "Data de termino deve ser maior que #{start_date_time}"
    end
